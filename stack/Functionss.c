...
void input_info(struct car **top)
{
    char* buf_string;
    struct car *pointer;
    do{
...
struct car* delete(struct car *top)
{
    if (!top)
    {
        printf("No cars:(\n");
    ...
    switch (input_check_int_for_sort()) {
        ...
        case 2:
            ...
            while(pointer)
            {
                ...
                {
                    case 1:
                        //for(int i = 0;i < 15 && pointer -> type.body_type[i];i++)
                            //fwrite(&pointer -> type.body_type[i],sizeof(pointer -> type.body_type[i]),1,file);
...
void load_from_file(struct car **top)
{
    FILE *file;
    struct car *pointer;
    struct car *temp = NULL;
    struct car *buf_pointer = NULL;
    char file_name[20];
    ...
    switch (input_check_int_for_sort()) {
        ...
        case 2:
            ...
            while(1)
            {
                ...
                temp = pointer;
                //fread(&new_line, sizeof(char), 1,file);
            }
            fclose(file);
            break;
        case 3:
